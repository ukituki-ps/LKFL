...
- **brief.md vs реализация: `cd` в targets** — brief.md указывал «Не использовать `cd` в targets»,
  но реализация использует `cd $(BACKEND_DIR) &&` для Go команд. Обоснование: mono-repo с
  `backend/` поддиректорией, альтернатива `GO111MODULE=on` с абсолютными путями менее читаема.
  Компромисс принят как рабочий вариант.
