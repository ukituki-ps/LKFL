...
- ⏳ **Не выполнено** — требует деплоя на serverDev и ручной проверки
...
- [ ] Config маршрутизирует `/api` → 18080 — ⏳ требует обновления на serverDev
- [ ] Config маршрутизирует `/realms`, `/protocol` и др. → 19081 — ⏳ требует обновления на serverDev
- [ ] Config маршрутизирует `/` → 8081 — ⏳ требует обновления на serverDev
...
1. **E2E тесты не выполнены** — требуется деплой на serverDev: git pull → docker compose down → docker compose up -d → проверка в браузере.
2. **External nginx на serverDev** — конфиг нужно обновить на сервере: убрать internal nginx mapping, добавить прямые порты 18080, 19081, 8081.
