...
/**
 * LKFL Logo — SVG-компонент, переиспользуемый в Keycloak и фронтенде.
 * Цвет определяется через `currentColor` родителя.
 *
 * Path из прототипа "ЛК физика(1).html", fill="currentColor".
 */
...
export function Logo({ size = 'md', className, style }: LogoProps) {
  ...
  return (
    <svg
      className={className}
      viewBox="126.006 216 243.994 68"
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
      height={height}
      style={{ width: 'auto', color: 'currentColor', ...style }}
    >
      <path
        clipRule="evenodd"
        fillRule="evenodd"
