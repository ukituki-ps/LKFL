...
-- Unique: tenant + email (email уникален в рамках tenant'а)
...
CREATE TABLE lkfl_platform.accounts (
    id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id       UUID NOT NULL UNIQUE REFERENCES lkfl_platform.users(id) ON DELETE RESTRICT,
...
-- Unique: user + role (одна роль на пользователя)
