...
func main() {
	...
	// Routes с префиксом /deploy-webhook (внешний nginx на serverPr01 не обрезает префикс)
	mux.HandleFunc("POST /deploy-webhook/deploy", handler.handleDeploy)
	mux.HandleFunc("POST /deploy-webhook/deploy/pr", handler.handleDeployPR)
	mux.HandleFunc("POST /deploy-webhook/rollback", handler.handleRollback)
