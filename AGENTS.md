...
- `doc/план/задачи.md` — реестр 68 задач: 67 doc (M00→M16) + 1 code (M17)
- Каждая задача: `doc/задачи/M{MM}-{slug}/T{MM}{NN}-{name}/`
- **Структура:** `brief.md` (контекст) + `plan.yaml` (чеклист) + `report.md` (отчёт)
- **Номинклатура:** `T{MM}{NN}` — MM = номер вехи, NN = порядковый номер
- **Никогда НЕ удалять** — даже отменённые задачи остаются

### Код

Старый план (4 фазы, 28 вех, 240 задач) удалён. Новая структура формируется после M17.

| Веха | Статус | Описание |
|------|--------|----------|
| **M17** | ⏳ | 5 code (T1701-T1705) — инфраструктура, backend, frontend, CI/CD, Observability |
